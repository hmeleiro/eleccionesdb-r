---
title: "Voto exterior (CERA)"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Voto exterior (CERA)}
...
participación y voto de los ciudadanos españoles que residen en el extranjero.
Estos votos se contabilizan de forma separada al voto nacional y se gestionan
a través de circunscripciones específicas.

El paquete **eleccionesdb** ofrece dos funciones para acceder a estos datos:

| Función | Devuelve |
|---|---|
| `get_cera_resumen()` | Resúmenes de participación CERA (censo, votos válidos, abstenciones…) |
| `get_cera_votos()` | Votos por partido en el exterior |
...
añadir columnas descriptivas (`eleccion_descripcion`, `territorio_nombre`)
junto a las columnas de ID. `get_cera_votos()` acepta además
`use_recode = TRUE` para que `partido_nombre` use la agrupación del recode.

---

## Resúmenes de participación CERA

`get_cera_resumen()` devuelve datos agregados de participación del voto
exterior: censo, participación, votos válidos, nulos, en blanco y abstenciones.
...
`get_cera_votos()` devuelve el desglose de votos por partido en las
circunscripciones CERA:
...
```{r grafico-comparativa, fig.width = 6, fig.height = 4}
...
Con `denormalize = TRUE` y `use_recode = TRUE`, podemos obtener los nombres
de partido directamente, sin necesidad de hacer joins manuales:
...
También se puede enriquecer con colores obteniendo el recode por separado:
...
```{r grafico-cera-partidos, fig.width = 7, fig.height = 4}
...
```{r grafico-evolucion-cera, fig.width = 8, fig.height = 4}
...
Ambas funciones (`get_cera_resumen()` y `get_cera_votos()`) aceptan estos
...
descriptivas. `get_cera_votos()` acepta además `use_recode = TRUE`:

| Parámetro | Funciones | Descripción |
|---|---|---|
| `denormalize` | Ambas | Añade `eleccion_descripcion` y `territorio_nombre` |
| `use_recode` | `get_cera_votos()` | `partido_nombre` usa la agrupación del recode |
...
- **Territorios y partidos**: datos maestros en `vignette("datos-maestros")`.
- **Análisis de resultados**: flujos completos con dplyr y ggplot2 en
  `vignette("analisis-resultados")`.
