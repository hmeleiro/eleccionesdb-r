---
title: "Territorios y partidos"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Territorios y partidos}
...
Este vignette explica cómo explorar las dos entidades maestras de la base de
datos: **territorios** (la jerarquía territorial española) y **partidos**
(incluyendo sus agrupaciones o *recode*).
...
Los territorios en EleccionesDB se organizan en una jerarquía de 7 niveles:
...
`codigo_provincia`, `codigo_municipio`, etc.) y un `parent_id` que permite
navegar hacia arriba en la jerarquía.

## Listar territorios

`get_territorios()` devuelve un listado paginado que puedes filtrar por tipo,
códigos jerárquicos y nombre:
...
`get_territorio()` devuelve todos los códigos jerárquicos de un territorio:

```{r territorio-detalle}
...
`get_territorio_hijos()` devuelve los hijos directos de un territorio,
permitiendo recorrer la jerarquía de arriba a abajo:
...
`get_partidos()` permite buscar partidos por siglas, denominación o por su
grupo de recode:
...
`get_partido()` devuelve el detalle completo de un partido, con los datos del
recode expandidos como columnas con prefijo `recode_`:
...
Los partidos sin grupo de recode asignado tendrán `partido_recode_id = NA` y
las columnas `recode_*` también serán `NA`.

## Agrupaciones de partidos (recode)

Los partidos se agrupan en **familias** (recode) que permiten agregar votos de
partidos locales o coaliciones bajo un mismo paraguas. Por ejemplo, las
distintas candidaturas del PSOE en cada comunidad autónoma se agrupan bajo el
recode "PSOE".

```{r partidos-recode-lista}
# Listar agrupaciones
get_partidos_recode()
#> # A tibble: 50 × 4
...
`get_partido_recode()` devuelve una **lista** con dos elementos:

- `$recode` — tibble de 1 fila con la información del grupo.
- `$partidos` — tibble con todos los partidos miembros.

Se usa una lista porque un grupo puede contener cientos o miles de partidos:

```{r partido-recode-detalle}
...
- **Análisis de resultados**: aprende a combinar estos datos maestros con
  resultados electorales en `vignette("analisis-resultados")`.
