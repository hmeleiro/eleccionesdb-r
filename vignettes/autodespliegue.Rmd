---
title: "Despliegue propio de la base de datos y la API"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Despliegue propio de la base de datos y la API}
...
Por defecto, **eleccionesdb** se conecta a la API pública de Spain Electoral
Project (`https://api.spainelectoralproject.com/`). Sin embargo, si necesitas
más control —por ejemplo, mayor rendimiento, acceso sin límites de tasa, o
datos adicionales propios— puedes levantar tu propia instancia de la base de
datos PostgreSQL y la API REST.
...
El repositorio [eleccionesdb-etl](https://eleccionesdb.spainelectoralproject.com/)
contiene un pipeline reproducible que descarga datos electorales oficiales,
los transforma y los carga en una base PostgreSQL.
...
Consulta la [documentación completa del ETL](https://eleccionesdb.spainelectoralproject.com/)
para requisitos previos (PostgreSQL, paquetes R) y opciones avanzadas.

## Paso 2: Levantar la API

El repositorio [eleccionesdb-api](https://eleccionesdb-api.spainelectoralproject.com/)
...
La API estará disponible en `http://localhost:8000`. Puedes verificarlo con:
...
Consulta la [documentación completa de la API](https://eleccionesdb-api.spainelectoralproject.com/)
para opciones de configuración, autenticación y despliegue en la nube.

## Paso 3: Configurar eleccionesdb

Una vez que tu API esté funcionando, configura el paquete para que apunte a
ella:
...
A partir de aquí, todas las funciones del paquete (`get_elecciones()`,
`get_resultados()`, etc.) usarán tu instancia de la API.
